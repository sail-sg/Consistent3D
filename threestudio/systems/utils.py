...
def parse_optimizer(config, model):
    ...
    #             args["lr"] /= config.scale
    #         threestudio.debug(f"optimizer params rescale as {config.params}")
    #     if hasattr(config, "args"):
    #         if "lr" in config.args:
    #             config.args["lr"] /= config.scale
    #         threestudio.debug(f"optimizer args rescale as {config.args}")
    if hasattr(config, "params"):
        params = [
        ...
        threestudio.info(f"Specify optimizer params: {config.params}")
    else:
    ...
    if config.name in ["FusedAdam"]:
        ...
        optim = Adan(params, **config.args)
        # threestudio.debug(f'optimizer {vars(optim)}')
    else:
